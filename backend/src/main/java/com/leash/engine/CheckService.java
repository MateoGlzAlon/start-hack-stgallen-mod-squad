...
public class CheckService {
    ...
    private record PolicyView(String instruction, List<Rule> rules, String uncertainty, List<String> guidance, String inactiveCode, String inactiveMessage) {}
    ...
    /**
     * @param policyId optional local policy to enforce; default is the mandate snapshot inside the event
    ...
    public Decision check(JsonNode event, String policyId, String runId, String source) {
        ...
        synchronized (lock(key)) {
            ...
            Decision d;
            try {
                d = decide(f, policyId, key, source);
            ...
            decisions.put(f.authorizationId(), d);
            return d;
        }
    }

    private Decision decide(PurchaseFacts f, String policyId, String key, String source) {
        PolicyView pv = policyView(f.event(), policyId);
        JsonNode auth = f.event().path("authorization");
        List<Evidence> ev = new ArrayList<>();

        // Guards: nothing else matters if the authority behind the purchase is gone.
        if (pv.inactiveCode() != null) return make(f, key, source, DENIED, List.of(pv.inactiveCode()), pv.inactiveMessage(), ev, List.of(), false);
        if (!"active".equals(auth.path("authority_status").asText("active"))) {
            return make(f, key, source, DENIED, List.of("authority_not_active"), "Declined: the authority behind this purchase is no longer active.", ev, List.of(), false);
        }
        if (!"active".equals(auth.path("card_status_at_attempt").asText("active"))) {
            return make(f, key, source, DENIED, List.of("card_not_active"), "Declined: the card is not active.", ev, List.of(), false);
        }
        ...
        for (Rule r : pv.rules()) {
            var res = evaluator.evaluate(r, view, f.billingChf(), days -> approvedSpend(key, f.timestamp(), days));
            checks.add(res);
        ...
        if (unknowns.isEmpty() && flags.isEmpty()) {
            ...
            return make(f, key, source, APPROVED, List.of("all_rules_passed"), msg, ev, checks, false);
        }

        // Judge, with a deterministic fallback.
        List<String> flagCodes = flags.stream().map(s -> s.substring(0, s.indexOf(':'))).toList();
        String why;
        ...
        if (!judge.available()) why = "judge_unavailable";
            ...
            try {
                LlmJudge.Answer a = judge.judge(judgeInput(f, pv, checks, view, flags, key), budget);
                return fromJudge(f, key, source, pv, a, unknowns, ev, checks);
            } catch (LlmException e) {
                log.warn("Judge failed for {}: {}", f.authorizationId(), e.getMessage());
                why = "judge_failed";
            }
        }
        String state = stateForUncertainty(pv.uncertainty());
        List<String> codes = new ArrayList<>(flagCodes);
        unknowns.forEach(c -> codes.add(codeOf(c.rule()).replace("rule_failed_", "unverified_")));
        codes.add(why);
        codes.add("uncertainty_policy_" + pv.uncertainty());
        List<String> reasons = new ArrayList<>();
        unknowns.forEach(c -> reasons.add(c.detail()));
        flags.forEach(s -> reasons.add(s.substring(s.indexOf(':') + 2)));
        ...
        return make(f, key, source, state, codes, msg, ev, checks, false);
    }

    private Decision fromJudge(PurchaseFacts f, String key, String source, PolicyView pv, LlmJudge.Answer a,
                               List<RuleEvaluator.Result> unknowns, List<Evidence> ev, List<RuleEvaluator.Result> checks) {
        String state = stateOf(a.decision());
        List<String> codes = new ArrayList<>(a.reasonCodes());
        String msg = a.customerMessage();
        if (APPROVED.equals(state) && a.manipulationSuspected()) {
            state = PENDING;
            codes.add("possible_manipulation");
            msg = "I stopped this purchase to ask you: " + msg;
        } else if (APPROVED.equals(state) && !unknowns.isEmpty()) {
            // The model may not approve past a fact we could not verify.
            state = stateForUncertainty(pv.uncertainty());
            codes.add("unverified_fact");
            codes.add("uncertainty_policy_" + pv.uncertainty());
            msg = "I could not verify: " + String.join("; ", unknowns.stream().map(RuleEvaluator.Result::detail).toList()) + ".";
        }
        ...
        return make(f, key, source, state, codes, msg, ev, checks, true);
    }
    ...
    public Decision failSafeStepUp(String authorizationId, String runKey, String source, String code, String message) {
        return decisions.computeIfAbsent(authorizationId, id -> new Decision(id, runKey == null ? "local" : runKey, source, PENDING,
    ...
    private PolicyView policyView(JsonNode event, String policyId) {
        if (policyId != null && !policyId.isBlank()) {
            Policy p = policies.get(policyId).orElseThrow(() -> new ApiException(HttpStatus.NOT_FOUND, "No policy " + policyId));
            ...
            return new PolicyView(p.instruction, p.hardRules, p.uncertaintyPolicy, p.guidance, code, msg);
        }
        ...
        return new PolicyView(m.path("instruction").asText(""), rules == null ? List.of() : rules, uncertainty,
    ...
    /** Final approvals only, over simulated time: (ts - days, ts]. */
    private BigDecimal approvedSpend(String key, Instant ts, int days) {
        Instant from = ts.minus(Duration.ofDays(days));
        BigDecimal sum = BigDecimal.ZERO;
        for (Decision d : decisions.values()) {
            if (!key.equals(d.runKey()) || !APPROVED.equals(d.state()) || d.purchaseTimestamp() == null) continue;
    ...
    private ObjectNode judgeInput(PurchaseFacts f, PolicyView pv, List<RuleEvaluator.Result> checks, JsonNode view, List<String> flags, String key) {
        ObjectNode in = Json.MAPPER.createObjectNode();
        ObjectNode pol = in.putObject("customer_policy");
        ...
        ArrayNode dc = in.putArray("deterministic_checks");
        for (RuleEvaluator.Result r : checks) {
            dc.addObject().put("rule", RuleEvaluator.describe(r.rule())).put("verdict", r.verdict()).put("actual", r.actual()).put("detail", r.detail());
        }
        ArrayNode fl = in.putArray("why_you_are_asked");
        flags.forEach(fl::add);
        ...
        for (JsonNode i : a.path("items")) copy(i, items.addObject(), "line_no", "item_id", "item_category", "quantity", "unit_price", "currency");
    ...
    private Decision make(PurchaseFacts f, String key, String source, String state, List<String> codes, String message,
        ...
        return new Decision(f.authorizationId(), key, source, state, Decision.visecaFor(state), codes, message, ev, checks, usedLlm,
