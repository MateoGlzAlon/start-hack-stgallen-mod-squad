...
- **UI:** Next.js 14 (App Router, TypeScript, Tailwind 3) in `frontend/`, minimal and responsive (bottom tab bar on phones, top nav on desktop, light/dark by system). Tabs: **Policies** (home: say it in words → what I understood, tighten to decline-when-unsure, revoke), **Inbox** (paused purchases, Approve/Decline → `/resolve`, polled every 4 s), **Activity** (all decisions with evidence and per-rule checks), **Try** (a **Test your policies with example purchases** section: pick a scenario, every example purchase attempt from `GET /purchase-attempts` is sent to `POST /check` in order with a fresh `run_id`/ids, against the policy made from that scenario's instruction (else all active policies), results as rows with the evidence and inline Approve/Decline for the asked ones; below it one-click presets and a form for a hand-made purchase). The nav bar has a **Load scenarios** button: `GET /scenarios` (Viseca's reference data, data pack fallback), then one `POST /policies` per scenario's `cardholder_instruction`, skipping instructions that already have an active policy; a banner shows the outcome and the Policies page refreshes. There is no UI to start a Viseca run (`POST /runs` is API-only, e.g. Swagger); the worker's decisions do show in Inbox/Activity. It only talks to `/api/*`, which `next.config.mjs` proxies to the backend (`BACKEND_URL`, fixed at build time: `http://backend:8080` in compose, `http://localhost:8080` for `cd frontend && npm run dev`); never to Viseca, no key in the browser. `make provision` starts it on `FRONTEND_PORT` (3000); after UI changes `make restart s=frontend`. Only ONE backend (worker) may run per team key: workers share Viseca's queue, so a second backend steals events.
...
**Endpoints** (full list with examples in Swagger UI): `GET /status`, `GET /actuator/health` · `POST|GET /policies`, `GET|PATCH|DELETE /policies/{id}` · `POST /check?policy_id=&run_id=` (body: an event or a poll envelope; no `policy_id` and no `mandate` = try all active policies), `POST /check/{authorization_id}/resolve` · `GET /decisions?run_id=&state=`, `GET /decisions/{authorization_id}` · `GET /scenarios` (Viseca's scenarios with the customer's instruction; data pack fallback) · `GET /purchase-attempts?scenario_id=` (the data pack's example purchase attempts as ready-to-send events, in replay order; Viseca serves attempts only inside a run) · `POST /runs`, `GET /runs/{run_id}` (Viseca run counters + our decisions for it). Errors are `{"error": {"status", "message"}}`. A decision has `state`, `viseca_decision`, `reason_codes`, `customer_message`, `evidence[]`, `checks[]` (per-rule verdicts), `used_llm`, `decided_by` (`engine` | `customer`), `policy_id` (the policy that decided), `posted_to_viseca`.
